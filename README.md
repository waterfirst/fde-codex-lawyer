# Local Python × Codex × 법제처 MCP — 파산전문 변호사를 위한 2시간 토탈 솔루션

일요일 2시간 동안 변호사 친구에게 **AI 도구를 분업시켜 일하는 법**을 알려주고,  
**변호사 노트북 로컬 Python 리더 + Codex + 법제처(law.go.kr) MCP**를 연결해 개인파산/개인회생 업무용 루틴을 함께 만든다.

핵심 원칙:

> 실제 사건 문서는 **변호사 노트북 밖으로 나가지 않는다.**  
> Python이 로컬에서 문서를 읽고·마스킹하고·구조화한 뒤, Codex는 그 결과물만 보고 중요 판단 후보를 정리한다.

이 프로젝트는 **OpenAI FDE 연습 프로젝트**다.  
...
1. Python으로 변호사 노트북 안에서 사건 문서를 읽고 핵심 메타정보를 뽑기
2. 개인정보를 로컬에서 마스킹한 뒤 Codex에게 사건 자료 정리와 산출물 생성을 지시하기
...
- **Python local reader = 눈**
  - 변호사 노트북 안에서 문서 목록, 텍스트, 날짜, 금액, 채권자 후보, 누락자료 후보를 추출한다.
  - 원문 파일은 외부로 올리지 않는다.
- **Codex = 손**
  - Python이 만든 마스킹 JSON/Markdown을 받아 폴더, 파일, Markdown, CSV, HTML, GitHub Pages, 반복 루틴을 만든다.
- **법제처 MCP = 근거**
...
- **NotebookLM/Gemini = 선택 옵션**
  - 익명 샘플 또는 완전 마스킹 자료에 한해 보조 독해용으로만 쓴다.
  - 실제 의뢰인 원문 업로드는 금지한다.
...
| 0:15–0:35 | 첫 성공 | Python 로컬 리더로 익명 샘플 문서 인덱싱 |
| 0:35–0:50 | 환경연결 | Codex·법제처 MCP 연결 확인 |
| 0:50–1:20 | 사건 루틴 | Codex가 마스킹 결과로 개인파산/회생 Intake 루틴 실행 |
...
- 실습은 익명 샘플 사건만 사용
- 실제 사건은 Python 로컬 리더가 의뢰인명, 주민번호, 주소, 계좌번호, 민감 가족정보를 먼저 제거
- Codex에는 원문이 아니라 `case_intake.json`, `document_index.csv`, `redacted_text.md` 같은 구조화 결과만 전달
- NotebookLM/Gemini 같은 외부 클라우드는 실제 의뢰인 원문에 사용하지 않음
